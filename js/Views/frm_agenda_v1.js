...
function carregarquerycalendario(uDataini,uDatafim){
	...
	if ($("#chkSoFerias").is(":checked")){
		if ($("#chkSoSuporte").is(":checked")){
			...
			querycalendarmx +="  ,mff.description as texto,cast(0 as bit) as naoconf,cast(0 as bit) as emcasa,cast(0 as bit) as reminder,cast(0 as numeric(10)) as nopat,'' as tipomx"	
			...
			querycalendarmx +=" else isnull((select dbo.Tts_RGB2RGB(cl.u_cormarc,'H') from cl (nolock) where cl.no=mx.clno and cl.u_cormarc <> 0),'" + corAgendamentos + "') end as color"
			querycalendarmx +=" ,Case when mx.u_naoconf=1 then '" + corAgendNaoConf + "' "
			querycalendarmx +=" 	when mx.u_tipomx='05 SUPORTE' then '" + corSuporte + "' "
			querycalendarmx +=" 	when mx.u_emcasa=1 then '" + corTTS + "' "
			...
			querycalendarmx +="  ,cast(mx.texto as char(8000)) as texto,mx.u_naoconf as naoconf,mx.u_emcasa as emcasa,u_reminder as reminder,mx.u_nopat as nopat,mx.u_tipomx as tipomx"	
			...
			querycalendarmx += "  ,'' as cliente"
			querycalendarmx +="  ,'Férias' as texto,cast(0 as bit) as naoconf,cast(0 as bit) as emcasa,cast(0 as bit) as reminder,cast(0 as numeric(10)) as nopat,'' as tipomx"	
		...
		querycalendar +=" ,pa.resumo as texto,cast(0 as bit) as naoconf,cast(0 as bit) as emcasa,cast(0 as bit) as reminder,cast(0 as numeric(10)) as nopat,'' as tipomx"	
		...
		querycalendarmx +="  ,vi.resumo as texto,cast(0 as bit) as naoconf,cast(0 as bit) as emcasa,cast(0 as bit) as reminder,cast(0 as numeric(10)) as nopat,'' as tipomx"	
...
function guardarnovoevento(calEvent){
	...
	if (tabela==="mx"){
		...
		query = "insert into mx (mxstamp,data,hinicio,hfim,inicio,fim,clnome,texto,nkeyid,ckeyid,tecnico,tecnnm,clno,clestab,morada,local,codpost,vendedor,vendnm,username,userno,tusername,tuserno,origem,u_naoconf,u_emcasa,u_reminder,u_fref,u_tipomx,ousrinis,ousrdata,ousrhora,usrinis,usrdata,usrhora)"
...
function duplicarregisto(calEvent){
	...
	if (tabela==="mx"){
		...
		query = "insert into mx (mxstamp,data,hinicio,hfim,inicio,fim,clnome,texto,nkeyid,ckeyid,cor,tecnico,tecnnm,clno,clestab,morada,local,codpost,origem,eorigem,descorigem,vendedor,vendnm,username,userno,tusername,tuserno,u_naoconf,u_duracao,u_emcasa,u_reminder,u_fref,u_tipomx,ousrinis,ousrdata,ousrhora,usrinis,usrdata,usrhora)"
