...
function guardarevento(calEvent){
	...
	if (tabela==="mx"){
		...
		",username=isnull((select top 1 username from us where us.tecnico=" + uNoTec.toString().trim() + "),'')" +
		",userno=isnull((select top 1 userno from us where us.tecnico=" + uNoTec.toString().trim() + "),0)" +
		",tusername=isnull((select top 1 username from us where us.tecnico=" + uNoTec.toString().trim() + "),'')" +
		",tuserno=isnull((select top 1 userno from us where us.tecnico=" + uNoTec.toString().trim() + "),0)" + 
		",u_fref='" + uProjeto + "'" +
		",u_tipomx='" + uTipomx + "'" +
...
function guardarnovoevento(calEvent){
	...
	if (tabela==="mx"){
		...
		query = "insert into mx (mxstamp,data,hinicio,hfim,inicio,fim,clnome,texto,nkeyid,ckeyid,tecnico,tecnnm,clno,clestab,morada,local,codpost,vendedor,vendnm,username,userno,tusername,tuserno,origem,u_naoconf,u_emcasa,u_fref,u_tipomx,ousrinis,ousrdata,ousrhora,usrinis,usrdata,usrhora)"
...
function duplicarregisto(calEvent){
	...
	if (tabela==="mx"){
		...
		query = "insert into mx (mxstamp,data,hinicio,hfim,inicio,fim,clnome,texto,nkeyid,ckeyid,cor,tecnico,tecnnm,clno,clestab,morada,local,codpost,origem,eorigem,descorigem,vendedor,vendnm,username,userno,tusername,tuserno,u_naoconf,u_duracao,u_emcasa,u_fref,u_tipomx,ousrinis,ousrdata,ousrhora,usrinis,usrdata,usrhora)"
		...
		query += " ," + (uEmCasa.toString().trim().toLowerCase() === "true" ? " 1 " : "0") + " as emcasa,'" + uProjeto + "','" + uTipomx + "'" 
		query += ",'" + m_chinis + "','" + dtoshoje() + "','" + time() + "','" + m_chinis+ "','" + dtoshoje() + "','" + time() + "'"
		query += " from mx (nolock) "
