//*******************************************************************
//** JavaScript para o ecrã de Lista de tarefas (ToDoList) da TTS
//** 
//** Criado por: Rui Vale  
...
var varmyTmptdl = "<tr data-type=jsonTarefas'>"
		...
		+ '<td><input type="text" name="txtnome" id="txtnome" class="form-control" value="{{:nome}}" data-campo="datac" style="width: 120px;" readonly></input></td>'
		...
		+ '<td><textarea data-campo="txtrel" rows="2" id="txtrel" class="form-control podealterar" placeholder="Relatório" style="width: 150px;" readonly>{{:rel}}</textarea></td>'
	...
	setTimeout(function(){
		...
		if ((modoedicao===undefined) || (modoedicao===null)) { 
			...
			if ((Stamp!==undefined) && (Stamp!==null) && (Stamp!=="")){
				var _datap = jQuery("#tabtdl tr.selected td:eq(6) > input").val();
				var _prioridade = jQuery("#tabtdl tr.selected td:eq(7) > div:nth-child(n) input").val();
				var _pquem = jQuery("#tabtdl tr.selected td:eq(8) input").val();
				var _equipamento = jQuery("#tabtdl tr.selected td:eq(9) input").val();
				var _tarefa = jQuery("#tabtdl tr.selected td:eq(10) textarea").val();
				var _relatorio = jQuery("#tabtdl tr.selected td:eq(14) textarea").val();

				waitwindow("A guardar!!")
				
				var query = "Update u_tdl set datap='" + _datap.toString().replace(/[-]/g,"") + "' "
				query += " ,prioridade=" + _prioridade.toString() + ",tecnnm='" + pub_tecnnm.toString().trim() + "',tecnico=" + pub_tecnico.toString().trim() + ",equipamento='" + _equipamento+ "',tarefa='" + _tarefa + "',rel='" + _relatorio + "' \n"			
						...
						if (json!==""){
							...
							for(var i in jsonTarefas){ 
								if(jsonTarefas[i].u_tdlstamp===Stamp){
									...
									jsonTarefas[i].tecnnm = pub_tecnnm;
									jsonTarefas[i].tecnico = pub_tecnico;
									jsonTarefas[i].equipamento = _equipamento;
									jsonTarefas[i].tarefa = _tarefa;
									jsonTarefas[i].rel = _relatorio;
								}
							}
...
function alterou_Qtd(){
	setTimeout(function(){
		var strqtd = jQuery("#tabtdl tr.selected td:eq(7) input").val();
		console.log(strqtd);
		...
		jQuery("#tabtdl tr.selected td:eq(7) input").val(qtd.toString());	
	},50)
}
...
function click_btfechar(linha){
	...
	if ((uStamp!==undefined) && (uStamp!==null) && (uStamp!=="")){
		...
		var query = "Update u_tdl set tecnnm='"+pub_tecnnm+"',tecnico="+pub_tecnico.toString().trim()+",fechada = 1,dataf='" + dtoshoje() + "',usrdata='" + dtoshoje() + "',usrhora='" + time() + "',usrinis='" + m_chinis+ "' Where u_tdlstamp='" + uStamp + "'"			
...
function novaTarefa() {
	...
	var cliente = jQuery("#Calendar_txtCliente").val();
	if(cliente===undefined || cliente===null || cliente==="" || nocliente===0){
		alertify.error("Primeiro tem de preencher o cliente!");
	}else{
...
jQuery(document).ready(function() {
	...
	jQuery.getScript( "../tts/js/TtsjsPHC.js?"+pub_versaoapp, function() {
		Tts_lerconfiguracoes(function(){
			...
			query = "select username,tecnnm,tecnico from us (nolock) where usercode='" + usercode + "'";
			...
			query = "select no,estab,nome,nome2,nome+'-'+nome2+'.'+cast(no as varchar)+'+'+cast(estab as varchar) as value from cl (nolock) order by 2"

			CorreNewWs("ExecQueryJson",query,function(json){
				if (json!==""){
					validou_clientes(json,function(){
						query = " select top 1 Case when cl.nome2='' then cl.nome else cl.nome2 end nome,clno "
						query += " from mx (nolock) "
						query += " inner join cl (nolock) on cl.no = mx.clno and cl.estab = mx.clestab "
						query += " where data='"+dtoshoje()+"' and tecnico=" + pub_tecnico.toString().trim()
						query += " order by (fim-inicio) desc "

						CorreNewWs("ExecQueryJson",query,function(json){
							if (json!==""){
								if(json.TbPhc.length>0 && json.TbPhc[0].nome!=="!#NA#!SEMDADOS"){
									nocliente = json.TbPhc[0].clno;
									setTimeout(function(){
										$("#Calendar_txtCliente").val(json.TbPhc[0].nome);										
									},200)
								}
							}else{
							}
			...
			query = "select distinct tpod,tpod as value from tpo (nolock) order by 1"
...
function validou_clientes(json,callback){
	for(var i in json.TbPhc){ 
		if(json.TbPhc[i].nome!=="!#NA#!SEMDADOS"){			
			jsonClientes.push(json.TbPhc[i]);			
		}else{
			jsonClientes.push();						
		}		
	}	
	
	var columnsCl = [{name: 'Nome',valueField: 'nome'},{name: 'Nº',valueField: 'no'},{name: 'Abrev.',valueField: 'nome2'}];
	
	$("#Calendar_txtCliente").ttsautocomplete({
		showHeader: false,
		columns: columnsCl,		
		source: jsonClientes,
		select: function(event, ui) {
			$('#Calendar_txtCliente').val((ui.item.nome2==="" ? ui.item.nome : ui.item.nome2));
			nocliente = ui.item.no;
			refrescarecran();
			return false;
        }
	});
...
function limpa_filtros(){
	...
	$("#Calendar_txtCliente").val("");
	$("#Calendar_cmdTecnico").val("");
	$("#Calendar_cmdFref").val("");
	$("#txtdtfecho").val(dt.toISOString().substring(0,10));
	$("#chkfechados").prop('checked', false);
...
function arranca_ecra(){
	...
	setTimeout(function(){
		...
		var nomecliente = jQuery("#Calendar_txtCliente").val();
		...
		if ((Fechados!==undefined) && (Fechados!==null) && (Fechados===true)){
			FiltroFechados = "And (u_tdl.fechada = 0 Or (u_tdl.fechada = 1 And u_tdl.dataf >= '" + uData + "'))"
		}

		if ((nomecliente===undefined) || (nomecliente===null) || (nomecliente.toString().trim()==="")){
			uFiltroCl = "cl.nome=cl.nome ";
		}else{
			uFiltroCl = "(cl.nome like '" + nomecliente.toString().trim() + "' ";
			uFiltroCl += " or cl.nome2 like '" + nomecliente.toString().trim() + "')";
		}
		...
		query = "Select *" 
			+	" From u_tdl (nolock) "
			+	" inner Join cl (nolock) on cl.no = u_tdl.no and cl.estab = u_tdl.estab "
			+	" Where " + uFiltroCl 
			+	FiltroFechados
			+	" Order by u_tdl.prioridade desc,u_tdl.id,u_tdl.datac,cast(u_tdl.tarefa as varchar(max))";
