...
var jqxhr = $.getJSON( "config.json?" + datahora, function(json) {
	usersql = json.usersql;	
	passsql = json.passsql;			
...
.done(function() {
	...
	pub_urlws = url + pub_urlws
	
})
.fail(function(jqXHR, textStatus, errorThrown) {
    /* alertify.error("Erro ao ler o ficheiro de configurações!!");
    var URL = "views/error_page.html";
    setTimeout(function(){ window.location = URL; }, 100);     */    
});

$.widget('custom.ttsautocomplete', $.ui.autocomplete, {
    ...
    _renderMenu: function(ul, items) {
        ...
        if (this.options.showHeader) {
            table=$('<div class="ui-widget-header col-sm-2" style="width:100%"></div>');
            // Column headers
...
function cria_datatable(idtabela,json,_template,procura,numlinhas,colunmDef,lang,callback){
	
	if ((colunmDef===undefined) || (colunmDef===null) || (colunmDef==="")){
		colunmDef = [];
	}
	...
	//console.log(colunmDef);

	if (lang!==""){
		CarregaTraducao(lang,function(LngObject){
			var _primeiro = LngObject["primeiro"]
			if ((_primeiro===undefined) || (_primeiro===null) || (_primeiro==="")){
				_primeiro = "Primeiro";
			}
			var _ultimo = LngObject["ultimo"]
			if ((_ultimo===undefined) || (_ultimo===null) || (_ultimo==="")){
				_ultimo = "Último";
			}
			var _seguinte = LngObject["seguinte"]
			if ((_seguinte===undefined) || (_seguinte===null) || (_seguinte==="")){
				_seguinte = "Seguinte";
			}
			var _anterior = LngObject["anterior"]
			if ((_anterior===undefined) || (_anterior===null) || (_anterior==="")){
				_anterior = "Anterior";
			}
			var _semdados = LngObject["semdados"]
			if ((_semdados===undefined) || (_semdados===null) || (_semdados==="")){
				_semdados = "Sem dados";
			}
			var _mostra = LngObject["mostra"]
			if ((_mostra===undefined) || (_mostra===null) || (_mostra==="")){
				_mostra = "Mostra";
			}
			var _registosporpagina = LngObject["registosporpagina"]
			if ((_registosporpagina===undefined) || (_registosporpagina===null) || (_registosporpagina==="")){
				_registosporpagina = "registos por página";
			}
			var _pagina = LngObject["pagina"]
			if ((_pagina===undefined) || (_pagina===null) || (_pagina==="")){
				_pagina = "página";
			}
			var _de = LngObject["de"]
			if ((_de===undefined) || (_de===null) || (_de==="")){
				_de = "de";
			}
			var _procurar = LngObject["procurar"]
			if ((_procurar===undefined) || (_procurar===null) || (_procurar==="")){
				_procurar = "Procurar";
			}
			...
			var table = jQuery("#"+idtabela).DataTable({
				...
				"language": {
					...
					"emptyTable": _semdados,
					"lengthMenu": _mostra + " _MENU_ " + _registosporpagina,
					"zeroRecords": _semdados,
					"info": _mostra + " " + _pagina + " _PAGE_ " + _de + " _PAGES_",
					"infoEmpty": _semdados,	
					"search": _procurar + ": _INPUT_"			
				},
				"autoWidth": true,
...
function u_Stamp() {
	...
	var tempo = time();
	result = pub_login.substr(0,3) + dtoshoje() + tempo.replace(/[:]/g,"") + rString
    return result;
}
...
//	pub_FileUpload = Base64.encode(reader.result);

//      alert(pub_FileUpload);
...
function CorreNewWs(Funcao,key,Query,callback){
    //console.log(Query);
    var uErro = false;
	...
	$.ajax({ url: pub_urlws + Funcao,
		...
		success: function(mtext) {
			//console.log("success");
			//console.log(mtext);
			//console.log(Base64.decode(mtext));
			var mytext = Base64.decode(mtext);
			...
			if(json.TbErros[0].Erro==="False"){
				...
				alertify.error(json.TbErros[0].MsgErro);
				console.log("Erro : " + json.TbErros[0].MsgErro);
				callback("");    
			}
		},
		error: function(data) {
			alertify.error("Erro ao aceder ao servidor");
			//console.log("Erro : Erro ao aceder ao servidor");
			callback("");    
		}
	});
...
function SavePosition(position){
    ...
    pub_login = $.cookie(pub_nomeapp + "_Login"); 
	if ((pub_login === undefined) || (pub_login === null) || (pub_login==="")){
		var URL = "login.html";
		setTimeout(function(){ window.location = URL; }, 100);        
	}else{
		var Base64={_keyStr:"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=",encode:function(e){var t="";var n,r,i,s,o,u,a;var f=0;e=Base64._utf8_encode(e);while(f<e.length){n=e.charCodeAt(f++);r=e.charCodeAt(f++);i=e.charCodeAt(f++);s=n>>2;o=(n&3)<<4|r>>4;u=(r&15)<<2|i>>6;a=i&63;if(isNaN(r)){u=a=64}else if(isNaN(i)){a=64}t=t+this._keyStr.charAt(s)+this._keyStr.charAt(o)+this._keyStr.charAt(u)+this._keyStr.charAt(a)}return t},decode:function(e){var t="";var n,r,i;var s,o,u,a;var f=0;e=e.replace(/[^A-Za-z0-9\+\/\=]/g,"");while(f<e.length){s=this._keyStr.indexOf(e.charAt(f++));o=this._keyStr.indexOf(e.charAt(f++));u=this._keyStr.indexOf(e.charAt(f++));a=this._keyStr.indexOf(e.charAt(f++));n=s<<2|o>>4;r=(o&15)<<4|u>>2;i=(u&3)<<6|a;t=t+String.fromCharCode(n);if(u!=64){t=t+String.fromCharCode(r)}if(a!=64){t=t+String.fromCharCode(i)}}t=Base64._utf8_decode(t);return t},_utf8_encode:function(e){e=e.replace(/\r\n/g,"\n");var t="";for(var n=0;n<e.length;n++){var r=e.charCodeAt(n);if(r<128){t+=String.fromCharCode(r)}else if(r>127&&r<2048){t+=String.fromCharCode(r>>6|192);t+=String.fromCharCode(r&63|128)}else{t+=String.fromCharCode(r>>12|224);t+=String.fromCharCode(r>>6&63|128);t+=String.fromCharCode(r&63|128)}}return t},_utf8_decode:function(e){var t="";var n=0;var r=c1=c2=0;while(n<e.length){r=e.charCodeAt(n);if(r<128){t+=String.fromCharCode(r);n++}else if(r>191&&r<224){c2=e.charCodeAt(n+1);t+=String.fromCharCode((r&31)<<6|c2&63);n+=2}else{c2=e.charCodeAt(n+1);c3=e.charCodeAt(n+2);t+=String.fromCharCode((r&15)<<12|(c2&63)<<6|c3&63);n+=3}}return t}}
		var _login = $.cookie(pub_nomeapp + "_Passw"); 
		...
		CorreNewWs("GuardaRegisto",sftkey("MAIN"),query,function(json){
			if (json!==""){
...
function LimpaFiltros(elemento,name){
    ...
    elemento.children().each(function() {
		...
		if ((atributos[0].id!==undefined) && (atributos[0].id!=="")){
			$.removeCookie(pub_nomeapp + "_" + name + "_" + atributos[0].id);
		}else{
			if ((atributos[0].name!==undefined) && (atributos[0].name!=="")){
				$.removeCookie(pub_nomeapp + "_" + name + "_" + atributos[0].name);
			}									
		}
...
function PreencheElementosFLT(elemento,name){
    ...
    elemento.children().each(function() {
		...
		if ((atributos[0].campo!=="") && (atributos[0].tabela!=="")){
			if ((atributos[0].id!==undefined) && (atributos[0].id!=="")){
				uValor=$.cookie(pub_nomeapp + "_" + name + "_" + atributos[0].id); 							
			}else{
				if ((atributos[0].name!==undefined) && (atributos[0].name!=="")){
					uValor=$.cookie(pub_nomeapp + "_" + name + "_" + atributos[0].name); 							
				}									
			}
        ...
        if ((atributos[0].campo!=="") && (atributos[0].tabela!=="")){
			...
			if (atributos[0].classe.trim().toUpperCase()!=="DATFIMFILTRO"){
				...
				if ((atributos[0].classe.trim().toUpperCase()==="CHKFILTRO") || (atributos[0].classe.trim().toUpperCase()==="CHKINVFILTRO")){						
					if ((atributos[0].id!==undefined) && (atributos[0].id!=="")){
						if (uValor===true){
							$.cookie(pub_nomeapp + "_" + name + "_" + atributos[0].id,"True"); 							
						}else{
							$.cookie(pub_nomeapp + "_" + name + "_" + atributos[0].id,"False"); 														
						}				
					}else{
						if ((atributos[0].name!==undefined) && (atributos[0].name!=="") && (uValor===true)){
							$.cookie(pub_nomeapp + "_" + name + "_" + atributos[0].name,"True"); 							
						}else{
							$.cookie(pub_nomeapp + "_" + name + "_" + atributos[0].id,"False"); 														
						}									
					}
					...
					if ((atributos[0].id!==undefined) && (atributos[0].id!=="")){
						if (valor!==undefined){
							$.cookie(pub_nomeapp + "_" + name + "_" + atributos[0].id,valor); 								
						}				
					}else{
						if ((atributos[0].name!==undefined) && (atributos[0].name!=="") && (valor!==undefined)){
							$.cookie(pub_nomeapp + "_" + name + "_" + atributos[0].name,valor); 							
						}									
					}
				}	
			}
    ...
    tabela.find('tr').each(function (rowIndex, r) {
        if (rowIndex!==0){
            $(this).find('th,td').each(function (colIndex, c) {
				...
				if (campo!==undefined){
					if (campo.toString().toLowerCase().replace("w_","")===bdtabela.toString().toLowerCase().replace("w_","")+"stamp") {
						uIndex=colIndex;
						//console.log(uIndex);
						return uIndex;
					}
...
function ValidaElementos(elemento,bdtabela){
    ...
    elemento.children().each(function() {
        ...
        if ((uCampo!=="") && (uTabela.toString().toUpperCase()===bdtabela.toString().toUpperCase())){       
            ...
            if (uCampo!==""){			
				if ((uTipo.toUpperCase()==="N") && ($.isNumeric(uValor)===false)){
					if (uName!==""){
						var txtValor = $("[name='"+uName+"']");
					}else{
						var txtValor = $("[id='"+uId+"']");
...
function OpenModalBoxSup(modalid,header, inner, bottom,semheader,mtop){
	if ((semheader===undefined) || (semheader===null) || (semheader.toString().trim==="")){
		semheader = true;
	}
	if ((mtop===undefined) || (mtop===null) || (mtop.toString().trim==="")){
		mtop = 9999999;
	}
	var strtop =  mtop.toString();

	var nExiste = jQuery('#'+modalid).length
	if (nExiste===0){
		var uModalBox = "<div id='" + modalid + "' class='modalboxsup'>"
		if (mtop !== 9999999){
			uModalBox += "	<div class='devoops-modal' style='top:" + strtop + "px;'>"
		}else{
...
function CloseModalBoxSup(modalid){
	var modalbox = jQuery('#'+modalid);
	modalbox.fadeOut('fast', function(){
		modalbox.find('.modal-header-name span').children().remove();
		modalbox.find('.devoops-modal-inner').children().remove();
...
function waitwindow(Texto){
    $.blockUI({message: "<h3><i class='fa fa-spinner fa-spin'></i>  " + Texto + "</h3>"});
}
