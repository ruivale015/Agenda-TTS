...
.done(function(json) {
	...
	//alert("AQUI "+passsql);	
/* 
    //Vou verificar se já foi feito o login
    pub_login = jQuery.cookie(pub_nomeapp + "_Login"); 
	
	if (pub_login === undefined || pub_login === undefined || pub_login === ""){
		var URL = "login.html";
		...
		pub_tecnico = jQuery.cookie(pub_nomeapp + "_Tecnico"); 
		pub_name = jQuery.cookie(pub_nomeapp + "_Name");
		pub_email = jQuery.cookie(pub_nomeapp + "_Email");
		pub_eadm = jQuery.cookie(pub_nomeapp + "_EAdm");			
	}
    ...
    if (pub_login === undefined || pub_login === undefined || pub_login === ""){
		...
		if (pub_passw === null || pub_passw === undefined || pub_passw===""){
			var URL = "login.html?" + pub_versaoapp;
			setTimeout(function(){ window.location = URL; }, 100);    
		}else{
			$('#NomeUser').html(pub_name);			
		}
    } */
})
...
function waitwindow(Texto){
    jQuery.blockUI({message: "<h3><i class='fa fa-spinner fa-spin'></i>  " + Texto + "</h3>"});
}
