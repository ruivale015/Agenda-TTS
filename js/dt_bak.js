...
var varmyTmplrgmo = '<tr data-type="rgmo" class="{{:(status.toString().toLowerCase().trim()==="2.em curso" ? "emcurso" : "")}} {{:(status.toString().toLowerCase().trim()==="9.fechado" ? "fechado" : "")}}">'
			...
			+ '<td>{{:tarefa}}</td>'
			+ '<td>{{:resumo}}</td>'
			+ '<td><textarea data-campo="txtdescricao" rows="2" id="txtdescricao" class="form-control" placeholder="Descrição">{{:descricao}}</textarea></td>'
...
function arranca_ecra(){
	...
	setTimeout(function(){
		if ((_jsPHCok===true) && (pub_bdados!=="") && (_numJseCssCarregados >= pub_numJseCss)){
			...
			query = "select tecnico,tecnnm,username,iniciais from us (nolock) where usercode='" + usercode + "'";
	...
	for(var i in json.TbPhc){ 
		if(json.TbPhc[i].tecnnm!=="!#NA#!SEMDADOS"){			
			...
			varmyTmplrh = '<tr data-type="rh"  class="{{:(status.toString().toLowerCase().trim()==="2.em curso" ? "emcurso" : "")}} {{:(status.toString().toLowerCase().trim()==="9.fechado" ? "fechado" : "")}}">'
						+ '<td class="center">'
						+ '		<div id="bt_rhalterar" onclick="click_bt_alterar_rh(jQuery(this))" class="btn btn-primary btn-sm bt_small {{:(responsavel.toLowerCase().trim()!=="' + username.toLowerCase().trim() + '" ? "invisible" : "default")}}" data-stamp="{{:u_ttartstamp}}">'
						+ '			<span><i class="fa fa-edit txt-default"></i></span>'
						+ '		</div>'
						+ '</td>'
						+ '<td class="center">'
						+ '		<div id="bt_apagar" onclick="click_bt_apagar_rh(jQuery(this))" class="bt_apagar btn btn-danger btn-sm bt_small {{:(responsavel.toLowerCase().trim()!=="' + username.toLowerCase().trim() + '" ? "invisible" : "default")}}" data-stamp="{{:u_ttartstamp}}">'
						+ '			<span><i class="fa fa-trash-o txt-default"></i></span>'
						+ '		</div>'
						+ '</td>'
						+ '<td>{{:data}}</td>'
...
function carregadados_prj(json,callback){
	...
	for(var i in json.TbPhc){ 
		if(json.TbPhc[i].ttastamp!=="!#NA#!SEMDADOS"){
			if (json.TbPhc[i].nivel===2){
				...
				saldodias = (diasprevistos - diasreais);
			}
	...
	jQuery("#txtsaldodias").val(saldodias.toFixed(2));
	if (saldodias < 0){
		jQuery("#txtsaldodias").css("color", "red");		
	}else{
		jQuery("#txtsaldodias").css("color", "darkgreen");				
	}
...
function cria_datatablehff(idtabela,json,_template,procura,numlinhas,colunaordenacao){
	...
	$('.dataTables_scrollBody').scroll(function(){
		console.log($('.fixedHeader-floating'));
	...
	$(function() {
		...
		fakeContainer.scroll(function() {
				console.log("fakeContainer.scroll 2");
		...
		$('.dataTables_scrollBody').scroll(function(){
				console.log("dataTables_scrollBody 2");
...
fakeContainer.scroll(function() {
	console.log("fakeContainer.scroll");
	tableContainer.scrollLeft(fakeContainer.scrollLeft());
	$('.fixedHeader-floating').scrollLeft($(this).scrollLeft());
});
...
$(window).scroll(function() {
	var seexiste = $(".fixedHeader-floating");
	if (seexiste.length!==0){
		seexiste.css("top","0px");
		seexiste.css("display","block");
		seexiste.css("block;","auto");
		seexiste.css("width","100% !important");
	}
});
...
function renderiza_Ecran(){	
	...
	setTimeout(function(){
		...
		$(function() {
			...
			fakeContainer.scroll(function() {
				console.log("fakeContainer.scroll 3");
				tableContainer.scrollLeft(fakeContainer.scrollLeft());
				$('.fixedHeader-floating').scrollLeft($(this).scrollLeft());
			});
		  
			$('.dataTables_scrollBody').scroll(function(){
				console.log("dataTables_scrollBody 3");
				fakeContainer.scrollLeft(tableContainer.scrollLeft());
				$('.fixedHeader-floating').scrollLeft($(this).scrollLeft());
			});		
		})
...
documento.on("click", "#limpa_filtros", function () {
...
//*******************************************************************
 ...
 function abre_ecra_tarefas(){
	...
	var form = jQuery('<form id="event_form">'+
		...
		'				<div class="col-lg-5 col-sm-4 col-xs-9 control-label autocomplete">'+
		'					<input type="text" id="txtlresumo" name="txtlresumo" value="' + jsonttapa[0].resumo + '" class="form-control input-sm" placeholder="Resumo" data-toggle="tooltip" data-placement="top" title="Resumo" readonly>'+
		'				</div>	'+
	...
	jQuery('#event_guardar').on('click', function(){	
		...
		query = "update ttapa set usnaresp='" + responsavel + "'"
